<x-app-layout>
  ...
  <form action="{{ route('recipe.store') }}" method="POST" class="w-10/12 p-4 mx-auto bg-white rounded" enctype="multipart/form-data">
    ...
    <div class="grid grid-cols-2 rounded border border-gray-500 my-4">
      ...
      <div class="col-span-1 p-4">
        <input type="text" name="title" value="{{ old('title') }}" placeholder="レシピ名" class="border border-gray-600 p-2 mb-4 w-full rounded">
      ...
      @foreach ($categories as $c)
        <option value="{{ $c['id'] }}" {{ ($recipe['category'] ?? null) == $c['id'] ? 'selected' : ''}}>{{ $c['name'] }}</option>
      @endforeach
        </select>
  ...
  @endif     
    ...
    <div class="flex justify-center">
      <x-primary-button class="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">レシピを投稿する</x-primary-button>
    </div>
    <hr class="my-4">
